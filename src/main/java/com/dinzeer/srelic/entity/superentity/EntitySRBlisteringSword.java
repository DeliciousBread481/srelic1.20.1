 ...
 * - 处理多部分实体（如末影龙部位）
 */
public class EntitySRBlisteringSword extends EntityAbstractSummonedSword {
    // 新增常量：追踪配置
    private static final int DEFAULT_SEARCH_COOLDOWN = 5; // 目标搜索冷却时间
    private static final float DEFAULT_TRACKING_RANGE = 8.0f; // 默认追踪范围
    private static final double DEFAULT_TRACKING_PRECISION = 0.98; // 默认追踪精度
    private static final double DEFAULT_MAX_PREDICTION = 10.0; // 最大预测距离
    
    // 新增常量：生成模式相关
    private static final double RANDOM_SPAWN_RANGE = 2.5; // 随机生成范围
    private static final double CIRCLE_SPAWN_RADIUS = 3.0; // 环绕生成半径
    
    // 数据同步参数（移除TRACKING）
    public static final EntityDataAccessor<Boolean> IT_FIRED = SynchedEntityData.defineId(EntitySRBlisteringSword.class, EntityDataSerializers.BOOLEAN);
    public static final EntityDataAccessor<Optional<UUID>> TARGET = SynchedEntityData.defineId(EntitySRBlisteringSword.class, EntityDataSerializers.OPTIONAL_UUID);
    public static final EntityDataAccessor<Float> SPEED = SynchedEntityData.defineId(EntitySRBlisteringSword.class, EntityDataSerializers.FLOAT);
    public static final EntityDataAccessor<Vector3f> CENTER_POS = SynchedEntityData.defineId(EntitySRBlisteringSword.class, EntityDataSerializers.VECTOR3);
    public static final EntityDataAccessor<Float> DIRECTION_YAW = SynchedEntityData.defineId(EntitySRBlisteringSword.class, EntityDataSerializers.FLOAT);
    public static final EntityDataAccessor<Float> DIRECTION_PITCH = SynchedEntityData.defineId(EntitySRBlisteringSword.class, EntityDataSerializers.FLOAT);
    public static final EntityDataAccessor<Float> DIY_YAW = SynchedEntityData.defineId(EntitySRBlisteringSword.class, EntityDataSerializers.FLOAT);
    public static final EntityDataAccessor<Float> DIY_PITCH = SynchedEntityData.defineId(EntitySRBlisteringSword.class, EntityDataSerializers.FLOAT);
    private static final EntityDataAccessor<Optional<UUID>> LOCK_TARGET = SynchedEntityData.defineId(EntitySRBlisteringSword.class, EntityDataSerializers.OPTIONAL_UUID);
    private static final EntityDataAccessor<Integer> LOCK_TARGET_ID = SynchedEntityData.defineId(EntitySRBlisteringSword.class, EntityDataSerializers.INT);
    private static final EntityDataAccessor<Integer> DELAY_TICK = SynchedEntityData.defineId(EntitySRBlisteringSword.class,EntityDataSerializers.INT);

    public static final EntityDataAccessor<Boolean> IT_CHANGED = SynchedEntityData.defineId(EntitySRBlisteringSword.class, EntityDataSerializers.BOOLEAN);
    ...
    // 新增追踪参数配置
    protected float trackingRange = DEFAULT_TRACKING_RANGE;
    protected LivingEntity dynamicTarget;
    protected boolean useSmartTracking = true; // 是否启用智能追踪

    // 新增：追踪精度控制
    protected double directTrackingPrecision = DEFAULT_TRACKING_PRECISION;
    protected double maxPredictionDistance = DEFAULT_MAX_PREDICTION;
    ...
    public EntitySRBlisteringSword(EntityType<? extends Projectile> type, Level world) {
        super(type, world);
    ...
    public static EntitySRBlisteringSword createInstance(PlayMessages.SpawnEntity packet, Level worldIn) {
        return new EntitySRBlisteringSword(SREntiteRegristrys.SRBlisteringSword, worldIn);
    }
    ...
    private float calculateEnchantedDamage(float baseDamage, Entity target1) {
        ...
        if (target1 instanceof LivingEntity target) {
            ...
            damage += typeBonus+target.getMaxHealth()*0.01f;
        }
        return damage;
    }
     ...
     * @param owner 飞剑所有者
     * @param world 游戏世界
     * @param centerPos 生成中心位置
     * @param mode 生成模式（CIRCLE/RANDOM）
     * @param count 生成数量
     * @param change 是否跟随视角变化
     * @param diyYaw 自定义Y轴角度
     * @param diyPitch 自定义X轴角度
     * @param zj 角度偏移量
     * @param k 角度增量
     * @param damage 基础伤害
    ...
    public static void spawnSwords(LivingEntity owner, Level world, Vec3 centerPos, SpawnMode mode, int count,
        ...
        for (int i = 0; i < count; i++) {
            EntitySRBlisteringSword sword = new EntitySRBlisteringSword(SREntiteRegristrys.SRBlisteringSword, world);
            ...
            switch (mode) {
                case RANDOM -> {
                    double x = centerPos.x + (world.random.nextDouble() - 0.5) * RANDOM_SPAWN_RANGE * 2;
                    double y = centerPos.y + world.random.nextDouble() * RANDOM_SPAWN_RANGE * 2;
                    double z = centerPos.z + (world.random.nextDouble() - 0.5) * RANDOM_SPAWN_RANGE * 2;
                    sword.setPos(x, y, z);
                }
                case CIRCLE -> {
                    double angle = Math.PI * 2 * i / count;
                    Vec3 pos = centerPos.add(
                        CIRCLE_SPAWN_RADIUS * Math.cos(angle), 
                        0, 
                        CIRCLE_SPAWN_RADIUS * Math.sin(angle)
                    );
