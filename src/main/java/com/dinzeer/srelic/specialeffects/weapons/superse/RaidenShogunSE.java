package com.dinzeer.srelic.specialeffects.weapons.superse;
...
public class RaidenShogunSE extends SpecialEffect {
    ...
    public static IStackManager eyeFieldManager = SRStacksReg.ResolveStacks;
    // 奥义·梦想真说
    ...
    public static void onSlash(LivingHurtEvent event) {
        ...
        // 每次攻击积累愿力
         eyeFieldManager.addStacks(player, 1);
        event.getEntity().invulnerableTime=0;
        // 附加雷元素伤害
        event.getEntity().hurt(player.damageSources().magic(), 3.0f*event.getEntity().getRandom().nextInt(eyeFieldManager.getCurrentStacks(player)));
    ...
    public static void onPlayerTick(TickEvent.PlayerTickEvent event) {
        ...
        if (event.phase == TickEvent.Phase.END && 
            ...
            // 诸愿百眼之轮效果
            int resolve = eyeFieldManager.getCurrentStacks(player);
    ...
    public static void onKill(LivingDeathEvent event) {
        if (event.getSource().getEntity() instanceof Player player && 
            ...
            eyeFieldManager.addStacks(player, 15);
        }
    }
    ...
    public static void activateMusouNoHitotachi(Player player) {
        ...
        // 重置并应用buff
        eyeFieldManager.addStacks(player, -resolve);
