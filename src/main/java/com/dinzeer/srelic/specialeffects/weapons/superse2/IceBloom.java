package com.dinzeer.srelic.specialeffects.weapons.superse2;
...
public class IceBloom extends SpecialEffect {
    ...
    public static void onAttack(LivingHurtEvent event) {
        ...
        if (currentStacks >= TRANSFORMATION_THRESHOLD && !TRANSFORMATION_STATES.containsKey(player)) {
            ...
            // 改变刀的TranslationKey
            SlashBladeUtil.getState( blade).setTranslationKey(ZHCNLangRegistry.CreateItemLang(SRelicBuiltInRegsitry.THIRD_BLUE_2));
    ...
    public static void onPlayerTick(TickEvent.PlayerTickEvent event) {
        ...
        if (timer <= 0) {
            ...
            // 恢复刀的TranslationKey
            ItemStack blade = player.getMainHandItem();
            SlashBladeUtil.getState( blade).setTranslationKey(ZHCNLangRegistry.CreateItemLang(SRelicBuiltInRegsitry.THIRD_BLUE_1));
    ...
    public static void onDoSlash(SlashBladeEvent.DoSlashEvent event) {
        ...
        if (SlashBladeUtil.hasSpecialEffect(player, SRSpecialEffectsRegistry.ICE_BLOOM.get())) {
            if (SlashBladeUtil.getState(player.getMainHandItem()).getTranslationKey().equals(ZHCNLangRegistry.CreateItemLang(SRelicBuiltInRegsitry.THIRD_BLUE_2))) {
