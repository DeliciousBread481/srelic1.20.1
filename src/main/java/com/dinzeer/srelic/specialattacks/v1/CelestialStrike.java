...
import com.dinzeer.srelic.registry.lang.ZHCNLangRegistry;
...
public class CelestialStrike {
    public static final Map<LivingEntity, Integer> eclipseStacks = com.dinzeer.srelic.specialeffects.weapons.superse2.CelestialStrike.eclipseStacks;
    ...
    public static void DoSlash(LivingEntity playerIn){
        if (playerIn instanceof Player player){
            if (player.getMainHandItem().getItem() instanceof ItemSlashBlade){
                ...
                if (state.getTranslationKey().equals(ZHCNLangRegistry.CreateItemLang(SRelicBuiltInRegsitry.SEVEN_1)) ){
                    ...
                    state.setTranslationKey(ZHCNLangRegistry.CreateItemLang(SRelicBuiltInRegsitry.SEVEN_2));
                    state.setModel(Srelic.prefix("model/ornament/seven/seven_2.obj"));
                }
                else
                    if (state.getTranslationKey().equals(ZHCNLangRegistry.CreateItemLang(SRelicBuiltInRegsitry.SEVEN_2))){
                        if (CELESTIAL_STRIKE.getCurrentStacks(player)==100){
                            ...
                            state.setTranslationKey(ZHCNLangRegistry.CreateItemLang(SRelicBuiltInRegsitry.SEVEN_1));
